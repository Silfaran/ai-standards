...
run_fixture() {
  ...
  local plan_rel="fake-docs/specs/board-set-title/board-set-title-plan.md"
  local prompt
  prompt="$(cat <<EOF
/build-plan $plan_rel
...
the context bundle under handoffs/board-set-title/, and attempt the first
...
EOF
  ...
  (
    ...
    if [ -n "${SMOKE_VERBOSE:-}" ]; then
      claude --print --max-turns 30 --output-format text <<<"$prompt" 2>&1 | tee "$claude_log" || claude_exit=$?
    else
      claude --print --max-turns 30 --output-format text <<<"$prompt" >"$claude_log" 2>&1 || claude_exit=$?
    fi
    exit $claude_exit
  ) || claude_exit=$?
