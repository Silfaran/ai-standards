## Precondition — plan target is OPTIONAL but must be resolved before loading context

The argument (plan file path or feature name) is optional. Resolve the target **before** reading any standards file:
...
- **If no target was provided**, do NOT read standards, invariants, CLAUDE.md, or any agent file yet. Read only the minimum needed to help the developer choose:
  ...
  2. `{docs-dir}/workspace.md` → locate the plans directory
  3. List the plan files under the plans directory (show feature name + status if available)

  Then show the available plans to the developer and ask which one to execute. **Stop and wait** — do not continue until they answer.

---

## Execution (only once a plan target is known)

Read the file `ai-standards/commands/build-plan-command.md` and follow its instructions exactly.
