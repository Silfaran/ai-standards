...
- **If no target was provided**, do NOT read standards, invariants, CLAUDE.md, or any agent file yet. Read only the minimum needed to help the developer choose:
  ...
  3. `{specs-dir}/INDEX.md` → list existing specs

  Then show the available specs to the developer and ask which one to update, along with a short note on **what** should change. **Stop and wait** — do not continue until they answer.
