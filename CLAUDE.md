...
- Critical-path sub-checklists: `ai-standards/standards/critical-paths/` — curated rule subsets per feature kind (CRUD endpoint, authorization-protected action, PII write, payment endpoint, LLM feature, file upload, signature feature, geo-search feature, PWA surface). Reviewer agents identify the matching paths from the diff and load only those instead of the full ~280-rule checklist. The full checklists remain authoritative; the paths are how the reviewer focuses. See `critical-paths/README.md` for the routing rules.
