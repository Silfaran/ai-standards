...
## Files Created
<!-- List every new file with its full path -->
- `src/Domain/Model/Task.php`

## Files Modified
<!-- List every modified file with its full path and a brief description of the change -->
- `config/services.yaml` — added handler tag for `CreateTaskCommandHandler`

## Key Decisions
<!-- Non-obvious decisions that the next agent needs to understand. Focus on "why", not "what" — the code shows the what. -->
- Used `fanout` exchange instead of `direct` because multiple consumers may subscribe in the future

## For the Next Agent
<!-- What the next agent should focus on — and what they can safely ignore -->
