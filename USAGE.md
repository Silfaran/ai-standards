...
The root `Makefile` in `ai-standards/` orchestrates every service in the workspace using the `BACKEND_SERVICES` and `FRONTEND_SERVICES` lists from `workspace.mk` (generated by `/init-project`). Run these from the `ai-standards/` directory.
