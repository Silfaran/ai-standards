...
- [backend] PHPUnit 13 rejects `createMock()` on `final` classes (`ClassIsFinalException`), which collides with backend.md's "every application service is `final readonly class`" rule whenever a sibling service must be mocked in a unit test → decide between (a) relaxing the standard to `readonly class` (keeps mocking simple, minor encapsulation loss) or (b) mandating a `{ServiceName}Interface` for every service so tests mock the interface (keeps `final`, adds one interface per service). Currently ambiguous — `backend-review-checklist.md` only enforces the one-public-method rule, not `final`, so the `readonly class` workaround passes review; surface a definitive decision in `backend.md` + checklist.
