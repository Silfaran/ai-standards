...
- [DevOps] `notification-worker` restart-loops on cold start because its compose entry lacks `depends_on: rabbitmq: condition: service_healthy` → add health-based `depends_on` on the worker (and any consumer service), or add a readiness-check retry in the worker entrypoint, so the worker waits for RabbitMQ's AMQP listener to be up instead of failing and looping.
- [Frontend Developer] Vue Test Utils auto-stub `stubs: { 'router-link': true }` does NOT render the default slot, so `wrapper.text()` misses text inside `<router-link>` → when a test must assert on the slot content of a router-link, use a custom stub like `{ 'router-link': { template: '<a><slot /></a>' } }` (pattern used in `DashboardBoardCard.spec.ts`).
