...
- [ ] **BE-069** — Domain exception factories (`static from*(...)`) MUST NOT accept Symfony or Doctrine types as parameters; they accept only PHP primitives (`int`, `string`, `\Throwable` when generic) or Domain types. Infrastructure-layer code performs the adaptation (extracting `$smtpCode`, `$reasonString`, etc.) before calling the Domain factory. Specialises BE-022 for the subtle case where the class itself is a pure Domain exception or VO but the factory bridges Domain ↔ Infrastructure (e.g. `EmailPermanentFailureException::fromTransport(\Symfony\Component\Mailer\Exception\TransportException $e)` or `FailureReason::fromException()` doing `$e instanceof \Symfony\Component\Messenger\Exception\UnrecoverableMessageHandlingException` — both leak Symfony imports into Domain). Surfaced in `comms-service.email-channel-adapter`.
...
- [ ] **BE-070** — When the idempotency key includes a time-bucketed component (`floor(unix_ts / N)`), the failure subscriber recomputing it MUST use the **command's** time (`$message->occurredAt` or equivalent), NOT the real clock. Handler-side compute and failure-side compute MUST produce the same hash on the same inputs — otherwise the subscriber cannot find the row to transition. Distinct from the `$now` used for the state-transition timestamp (which IS the real clock — the failure happened now). Surfaced in `comms-service.email-channel-adapter`.
- [ ] **BE-071** — Aggregate state transitions on async failure (`markRetrying`, `markDeadLettered`, `markFailed`, etc.) MUST run inside a synchronous `WorkerMessageFailedEvent` listener — the row is updated BEFORE the framework acks the message to the failure transport. Post-worker hooks, batch reconciliation jobs, or cron sweeps leave orphan `pending` rows whose state diverges from the broker's view. Surfaced in `comms-service.email-channel-adapter`.
- [ ] **BE-072** — `UnrecoverableMessageHandlingException` wrapping MUST pass the original exception as `$previous` (`new UnrecoverableMessageHandlingException($e->getMessage(), 0, $e)`); failure-reason mappers (e.g. `FailureReason::fromException()`) MUST unwrap `getPrevious()` before `instanceof` classification — otherwise every wrapped failure is classified as `unknown:UnrecoverableMessageHandlingException` and the row's `failure_reason` loses the actual cause. Surfaced in `comms-service.email-channel-adapter`.
